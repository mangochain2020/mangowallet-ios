...
+ (void)requestWithType:(requestType )type
    ...
    if (![[NetworkUnit getInternetStatus] isEqualToString:@"notReachable"]) { // 有网络
        if (type == 1) { // GET 请求
            ...
            [_manager GET:URL parameters:parameter headers:nil progress:^(NSProgress * _Nonnull downloadProgress){
            ...
            [_manager POST:URL parameters:parameter headers:nil progress:^(NSProgress * _Nonnull uploadProgress){
...
parameter
    ...
    if (type == 1) {
        ...
        NSURLSessionDataTask *taskID = [_manager GET:URL parameters:parameter headers:nil progress:nil success:^(NSURLSessionDataTask * _Nonnull task, id  _Nullable responseObject){
        ...
        NSURLSessionDataTask *taskID = [_manager POST:URL parameters:parameter headers:nil progress:nil success:^(NSURLSessionDataTask * _Nonnull task, id  _Nullable responseObject){
