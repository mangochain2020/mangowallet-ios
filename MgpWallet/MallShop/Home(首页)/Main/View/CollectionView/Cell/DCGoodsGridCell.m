...
- (void)layoutSubviews
{
    ...
    [_button mas_makeConstraints:^(MASConstraintMaker *make) {
        make.bottom.top.right.left.mas_equalTo(0);
    }];
...
- (void)setGridItem:(DCGridItem *)gridItem
{
    ...
    _gridLabel.text = gridItem.cateName;
...
//    if (_gridItem.pic.length == 0) return;
    if ([[_gridItem.pic substringToIndex:4] isEqualToString:@"http"]) {
        [_gridImageView sd_setImageWithURL:[NSURL URLWithString:gridItem.pic]placeholderImage:[UIImage imageNamed:@"default_49_11"]];
    }else{
