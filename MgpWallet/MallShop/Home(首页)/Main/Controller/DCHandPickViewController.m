...
- (UICollectionView *)collectionView
{
    if (!_collectionView) {
        ...
        _collectionView.frame = CGRectMake(0, -STATUSBAR_HEIGHT, ScreenW, ScreenH+STATUSBAR_HEIGHT);
...
- (void)viewDidLoad {
    ...
    //today program differ soda inject embody vicious town abuse soul art proud
    [self getNetwork];
...
{
    WEAKSELF
    [[MGPHttpRequest shareManager]post:@"/appStoreCategory/getCategoryProduct" paramters:@{@"lang":@"zh_CN",@"page":@"1",@"limit":@"10"} completionHandler:^(id  _Nonnull responseObj, NSError * _Nonnull error) {
...
{
    _topToolView = [[DCHomeTopToolView alloc] initWithFrame:CGRectMake(0, -SafeAreaBottomHeight, ScreenW, SafeAreaTopHeight)];
    WEAKSELF
...
- (NSInteger) numberOfSectionsInCollectionView:(UICollectionView *)collectionView {
    return 3+self.handModel.proList.count;
}
...
- (NSInteger)collectionView:(UICollectionView *)collectionView numberOfItemsInSection:(NSInteger)section {
    ...
    switch (section) {
        case 0:
            return self.handModel.cateList.count;
            break;
        case 1:
            return 1;
            break;
        case 2:
            return self.handModel.hot.list.count;
            break;
        default:
            return self.handModel.proList[section-3].list.count;
            break;
    }
    return 0;
}
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (indexPath.section == 0) {
        ...
        [cell.button addTarget:self action:@selector(lookSameClick:) forControlEvents:UIControlEventTouchUpInside];
        CateListItem *model = self.handModel.cateList[indexPath.row];
        DCGridItem *item = [DCGridItem new];
        ...
        cell.handheldImage = self.handModel.homeImg;
        gridcell = cell;
        ...
        ProListItem *proModel = self.handModel.hot.list[indexPath.row];;
        DCRecommendItem *model = [DCRecommendItem new];
        model.image_url = proModel.image_url.firstObject;
        model.price = [NSString stringWithFormat:@"%.2f",proModel.price];
        model.goods_title = proModel.storeName;
        cell.youLikeItem = model;
        gridcell = cell;
        ...
        ProListItem *proModel = self.handModel.proList[indexPath.section-3].list[indexPath.row];;
...
- (UICollectionReusableView *)collectionView:(UICollectionView *)collectionView viewForSupplementaryElementOfKind:(NSString *)kind atIndexPath:(NSIndexPath *)indexPath {
    ...
    if (kind == UICollectionElementKindSectionHeader){
        if (indexPath.section == 0) {
            ...
            headerView.imageGroupArray = self.handModel.sysSlider;
            ...
            [headerView.likeImageView sd_setImageWithURL:[NSURL URLWithString:self.handModel.hot.pic]];
            reusableview = headerView;
        }else if (indexPath.section >= 3 && self.handModel.proList[indexPath.section-3].list.count > 0){
            ProList *proList = self.handModel.proList[indexPath.section-3];
            DCYouLikeHeadView *headerView = [collectionView dequeueReusableSupplementaryViewOfKind:UICollectionElementKindSectionHeader withReuseIdentifier:DCYouLikeHeadViewID forIndexPath:indexPath];
            [headerView.likeImageView sd_setImageWithURL:[NSURL URLWithString:proList.pic]];
            reusableview = headerView;
        }
    ...
    if (kind == UICollectionElementKindSectionFooter) {
        ...
        if (2+self.handModel.proList.count == indexPath.section) {
...
- (CGSize)collectionView:(UICollectionView *)collectionView layout:(UICollectionViewLayout*)collectionViewLayout referenceSizeForHeaderInSection:(NSInteger)section {
    ...
    if (section == 0) {
        ...
        return self.handModel.hot.list.count > 0 ? CGSizeMake(ScreenW, 40) : CGSizeZero;
    }else if (section >= 3 && self.handModel.proList[section-3].list.count > 0){
...
- (CGSize)collectionView:(UICollectionView *)collectionView layout:(UICollectionViewLayout*)collectionViewLayout referenceSizeForFooterInSection:(NSInteger)section {
     ...
     if (2+self.handModel.proList.count == section) {
         return CGSizeMake(ScreenW, 40);
     }
...
- (void)lookSameClick:(UIButton *)button{
    CateListItem *model = self.handModel.cateList[button.tag];
    DCGoodsSetViewController *goodSetVc = [[DCGoodsSetViewController alloc] init];
    goodSetVc.cateId = [NSString stringWithFormat:@"%ld",(long)model.cateID];
...
//    _topToolView.hidden = (scrollView.contentOffset.y < STATUSBAR_HEIGHT) ? YES : NO;//判断顶部工具View的显示和隐形
