...
- (void)drawDashLine:(CGRect)rect {
    ...
    CGContextFillRect(ctx, CGRectMake(0, 2, self.bounds.size.width, self.bounds.size.height - 2));
...
- (void)drawTimeLineMask:(CGContextRef)ctx rect:(CGRect)rect {
    ...
    NSDictionary *attribute = @{NSFontAttributeName:[UIFont systemFontOfSize:10],NSForegroundColorAttributeName:[UIColor YYStock_textColor]};
    ...
    attribute = @{NSFontAttributeName:[UIFont systemFontOfSize:10],NSForegroundColorAttributeName:[UIColor YYStock_textColor]};
    ...
    attribute = @{NSFontAttributeName:[UIFont systemFontOfSize:10],NSForegroundColorAttributeName:[UIColor YYStock_topBarSelectedTextColor]};
    ...
    //绘制选中成交量
    drawText = NSLocalizedString(@"成交：", nil);
    attribute = @{NSFontAttributeName:[UIFont systemFontOfSize:10],NSForegroundColorAttributeName:[UIColor YYStock_textColor]};
    textRect = [self rectOfNSString:drawText attribute:attribute];
    drawRect = CGRectMake(50 + 280 , (rect.size.height - textRect.size.height)/2.f, textRect.size.width, textRect.size.height);
    [drawText drawInRect:drawRect withAttributes:attribute];
    ...
    if (wVolume > 1) {
        ...
        if (yVolume > 1) {
            drawText = [NSString stringWithFormat:@"%.2f %@",yVolume,NSLocalizedString(@"亿", nil)];
        } else {
            drawText = [NSString stringWithFormat:@"%.2f %@",wVolume,NSLocalizedString(@"万", nil)];
        }
    } else {
        drawText = [NSString stringWithFormat:@"%.2f %@",volume,NSLocalizedString(@"", nil)];
    }
    textRect = [self rectOfNSString:drawText attribute:attribute];
...
- (void)drawLineMask:(CGContextRef)ctx rect:(CGRect)rect {
    ...
    NSString *drawText = [self.selectedModel DayDatail];
    CGRect textRect = [self rectOfNSString:drawText attribute:attribute];
    CGRect drawRect= CGRectMake(YYStockTimeLineViewLeftGap, (rect.size.height - textRect.size.height)/2.f + 3, textRect.size.width, textRect.size.height);
    [drawText drawInRect:drawRect withAttributes:attribute];
    ...
    [self.drawKlineDescTexts enumerateObjectsUsingBlock:^(NSString *  _Nonnull drawText, NSUInteger idx, BOOL * _Nonnull stop) {
        CGRect textRect = [self rectOfNSString:drawText attribute:attribute];
        CGRect drawRect = CGRectMake(90 + gap * idx , (rect.size.height - textRect.size.height)/2.f+ 3, textRect.size.width, textRect.size.height);
        [drawText drawInRect:drawRect withAttributes:attribute];
    }];
    ...
    [[self drawKlineTexts] enumerateObjectsUsingBlock:^(NSString *  _Nonnull drawText, NSUInteger idx, BOOL * _Nonnull stop) {
        CGRect textRect = [self rectOfNSString:drawText attribute:attribute];
        CGRect drawRect = CGRectMake(90 + 20 + gap * idx , (rect.size.height - textRect.size.height)/2.f+ 3, textRect.size.width, textRect.size.height);
        [drawText drawInRect:drawRect withAttributes:attribute];
    }];
    ...
    attribute = @{NSFontAttributeName:[UIFont systemFontOfSize:10],NSForegroundColorAttributeName:[UIColor YYStock_textColor]};
    textRect = [self rectOfNSString:drawText attribute:attribute];
    drawRect = CGRectMake(90 + 285 , (rect.size.height - textRect.size.height)/2.f+ 3, textRect.size.width, textRect.size.height);
    [drawText drawInRect:drawRect withAttributes:attribute];
    ...
    if (wVolume > 1) {
        ...
        CGFloat yVolume = wVolume/10000.f;
        if (yVolume > 1) {
            drawText = [NSString stringWithFormat:@"%.1f %@",yVolume,NSLocalizedString(@"亿", nil)];
        } else {
            drawText = [NSString stringWithFormat:@"%.1f %@",wVolume,NSLocalizedString(@"万", nil)];
        }
    } else {
        drawText = [NSString stringWithFormat:@"%.2f",volume];
    }
    textRect = [self rectOfNSString:drawText attribute:attribute];
    drawRect = CGRectMake(90 + 320 , (rect.size.height - textRect.size.height)/2.f+ 3, textRect.size.width, textRect.size.height);
    [drawText drawInRect:drawRect withAttributes:attribute];
}
...
- (NSArray *)drawTimeLineDescTexts {
    if (!_drawTimeLineDescTexts) {
        _drawTimeLineDescTexts = @[NSLocalizedString(@"price:", nil),NSLocalizedString(@"change:", nil),NSLocalizedString(@"均价:", nil)];
    }
