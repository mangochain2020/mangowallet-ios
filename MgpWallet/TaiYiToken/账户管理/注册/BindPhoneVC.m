...
-(void)initHeadView{
    ...
    _titlelabel.text = NSLocalizedString(@"绑定手机", nil);
...
-(void)countryCodeSelect{
    ...
    [CountryCodeVC toReturnCountryCode:^(NSString *countryCodeStr) {
        //在此处实现最终选择后的界面处理
        weakSelf.countryCode = countryCodeStr;
        [weakSelf.countryCodeBtn setTitle:countryCodeStr forState:UIControlStateNormal];
    }];
...
-(void)confirm{
    ...
    [self.view showHUD];
    MJWeakSelf
    [NetManager BindMobileWithMobile:phonenum VerifyCode:verifycode completionHandler:^(id responseObj, NSError *error) {
...
-(void)sendVerifyCode{
    ...
    [self openCountdown];
    MJWeakSelf
    [NetManager SendVerifyCodeWithType:SMS_Code MobileORMail:phonenum  CheckDevice:NO SafeCode:@"" completionHandler:^(id responseObj, NSError *error) {
//        dispatch_async_on_main_queue(^{
