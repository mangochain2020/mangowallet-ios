...
+ (NSData *)getBytesForSignature:(NSData *)chainId andParams:(NSDictionary *)paramsDic andCapacity:(int)capacity{
    ...
    for (NSMutableDictionary *actionDic in actions) {
...
///add
+ (NSData *)getBytesForSignature:(NSData *)chainId andParams:(NSDictionary *)paramsDic andPubKey:(NSArray *)pubkeyarray andCapacity:(int)capacity{
    EosByteWriter *writer = [[EosByteWriter alloc] initWithCapacity:capacity];
    if (chainId) {
        [writer putBytes:chainId ];
    }
    ...
    NSArray *pubkeys = pubkeyarray;
    [writer putVariableUInt:pubkeyarray.count];
    
    NSString *pubkey = pubkeys[0];
    [writer putLongLE:[EosUtils string_to_long:pubkey]];
