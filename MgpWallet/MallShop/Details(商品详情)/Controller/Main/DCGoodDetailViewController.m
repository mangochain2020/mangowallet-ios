...
//    [self setUpNav];
    ...
    [[MGPHttpRequest shareManager]post:@"/appStoreProduct/browse" paramters:@{@"productId":self.goodId} completionHandler:^(id  _Nonnull responseObj, NSError * _Nonnull error) {}];
...
//    NSArray *titles = @[@"商品",@"详情",@"评价"];
    NSArray *titles = @[NSLocalizedString(@"商品", nil),NSLocalizedString(@"详情", nil)];
    CGFloat margin = 5;
...
-(void)setUpChildViewControllers
{
    ...
    goodBaseVc.goodId = _goodId;
    goodBaseVc.storeUnit = self.storeUnit;
    goodBaseVc.postage = self.postage;
    goodBaseVc.proModel = self.proModel;
    ...
    DCParticularsShowViewController *goodCommentVc = [[DCParticularsShowViewController alloc] init];
    goodCommentVc.particularUrl = self.proModel.storeInfo;
...
- (void)setUpLeftTwoButton
{
    ...
    CGFloat buttonH = 50;
    CGFloat buttonY = self.view.bounds.size.height - buttonH-64;
...
- (void)setUpRightTwoButton
{
    ...
    NSArray *titles = @[NSLocalizedString(@"加入购物车", nil),NSLocalizedString(@"立即购买", nil)];
    CGFloat buttonW = ScreenW * 0.6 * 0.5;
    CGFloat buttonH = 50;
    CGFloat buttonY = ScreenH - buttonH - 64;
    for (NSInteger i = 0; i < titles.count; i++) {
        UIButton *button = [UIButton buttonWithType:UIButtonTypeCustom];
        button.titleLabel.font = PFR16Font;
...
//        });
        ...
        sureOrderVc.showPriceStr = self.proModel.price;
        sureOrderVc.showShopStr = self.proModel.storeName;
        sureOrderVc.expressagePriceStr = [NSString stringWithFormat:@"%ld",self.proModel.postage];
//        sureOrderVc.iconimage = self.proModel.image_url.firstObject;
        sureOrderVc.buyNum = 1;
        sureOrderVc.standard = self.proModel.storeType;
        sureOrderVc.proModel = self.proModel;
