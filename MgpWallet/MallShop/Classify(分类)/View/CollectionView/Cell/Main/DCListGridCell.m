...
- (void)setUpUI
{
    ...
    _gridImageView.contentMode = UIViewContentModeRedraw;
    ...
    NSInteger pNum = 0;// arc4random() % 10000;
    _commentNumLabel.text = [NSString stringWithFormat:@"%zd人已浏览",pNum];
...
//    _priceLabel.text = [NSString stringWithFormat:@"¥ %.2f",[youSelectItem.price floatValue]];
    _priceLabel.text = [NSString stringWithFormat:@"%@%.2f",[CreateAll GetCurrentCurrency].symbol,([[CreateAll GetCurrentCurrency].price doubleValue] * [youSelectItem.price doubleValue])];
    
    _gridLabel.text = youSelectItem.main_title;
    _commentNumLabel.text = [NSString stringWithFormat:@"%@人已浏览",youSelectItem.stock];
