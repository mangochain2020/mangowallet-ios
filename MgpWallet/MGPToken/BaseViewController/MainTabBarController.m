...
@property (nonatomic, strong) MallstoreADView *mallstoreADView;
...
- (void)viewWillAppear:(BOOL)animated{
    ...
    [[NSNotificationCenter defaultCenter]addObserver:self selector:@selector(posted:) name:UIApplicationDidBecomeActiveNotification object:nil];
    
    [[NSNotificationCenter defaultCenter]addObserver:self selector:@selector(DidEnterBackground) name:UIApplicationDidEnterBackgroundNotification object:nil];

}
- (void)DidEnterBackground{
    if (self.mallstoreADView) {
        [self.mallstoreADView.cancelBtn sendActionsForControlEvents:UIControlEventTouchUpInside];
    }
}
- (void)posted:(NSNotification*)s{
    ...
    if ([shareStr containsString:@"👉MangoWallet👈"]) {
        [[MGPHttpRequest shareManager]post:@"/appStoreProduct/find" paramters:@{@"proId":[shareStr componentsSeparatedByString:@"👈"].firstObject} completionHandler:^(id  _Nonnull responseObj, NSError * _Nonnull error) {

            if ([responseObj[@"code"]intValue] == 0) {

                ProListItem *model = [ProListItem mj_objectWithKeyValues:responseObj[@"data"]];
                NSString *temp = [shareStr componentsSeparatedByString:@"👈"][1];
                NSString *tempLast = [temp componentsSeparatedByString:@"👉"].lastObject;
                NSString *mgpName = [tempLast base64DecodingString];
                if (![[MGPHttpRequest shareManager].curretWallet.address isEqualToString:mgpName] && self.mallstoreADView==nil) {
                    self.mallstoreADView = [MallstoreADView dc_viewFromXib];
...
- (NSArray *)viewControllersForTabBar {
    ...
    UIViewController *thirdNavigationController = [[CYLBaseNavigationController alloc]
                                                    initWithRootViewController:[[DCHandPickViewController alloc] init]];
    
    UIViewController *fourthNavigationController = [[CYLBaseNavigationController alloc]
                                                  initWithRootViewController:[[UserInfoVC alloc] init]];
...
- (NSArray *)tabBarItemsAttributesForTabBar {
    ...
    NSDictionary *firstTabBarItemsAttributes = @{
                                                 CYLTabBarItemTitle : NSLocalizedString(@"钱包", nil),
...
//                                                 CYLTabBarLottieSize: [NSValue valueWithCGSize:CGSizeMake(22, 22)]
    ...
    NSDictionary *secondTabBarItemsAttributes = @{
                                                  CYLTabBarItemTitle : NSLocalizedString(@"应用", nil),
                                                  CYLTabBarItemImage : [UIImage imageNamed:@"mycity_normal"],
                                                  CYLTabBarItemSelectedImage : @"mycity_highlight",
                                                  CYLTabBarItemTitlePositionAdjustment: [NSValue valueWithUIOffset:UIOffsetMake(0, -3.5)],
    ...
    NSDictionary *thirdTabBarItemsAttributes = @{
                                                CYLTabBarItemTitle : NSLocalizedString(@"商城", nil),
